defmodule ExqUIWeb.DeadLive do
  ...
  defp jobs_details(page) do
    ...
    {:ok, jobs} =
      Api.failed(Exq.Api, score: true, offset: @page_size * (current_page - 1), size: @page_size)

    jobs =
      Enum.map(jobs, fn {job, score} ->
        {epoch, ""} = Float.parse(score)
        failed_at = DateTime.from_unix!(round(epoch))
        %{job: job, score: score, failed_at: failed_at}
      end)
